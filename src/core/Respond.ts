...
export type Trasnformer<T, N> = (body: T | undefined, status: Status, headers: HttpHeader[]) => MaybePromise<Respond<N>>

export function response<T>(body: T | undefined): Respond<T>
export function response<T>(body: T | undefined, status: Status): Respond<T>
export function response<T>(body: T | undefined, status: Status, ...headers: HttpHeader[]): Respond<T>
export function response<T>(res: Respond<T>, ...headers: HttpHeader[]): Respond<T>
export function response<T, N>(res: Respond<T>, transformer: (body: T | undefined, status: Status, headers: HttpHeader[]) => Respond<N>): Respond<N>
export function response<T, N>(res: Respond<T>, transformer: (body: T | undefined, status: Status, headers: HttpHeader[]) => Promise<Respond<N>>): Promise<Respond<N>>
export function response<T, N>(arg1: T | undefined | Respond<T>, arg2?: Status | Trasnformer<T, N> | HttpHeader, ...headers: HttpHeader[]) {
