...
type ComputeStream<T> = {
    map<R>(
        ...
        T extends Promise<infer _M> ? R extends Promise<infer _AwaitedR> ? R
        : Promise<R>
            : R
    >;
    mapNN<R>(
...
export function computeStream<T>(initValue: T): ComputeStream<T> {
    function map<R>(
        callbackFn: T extends Promise<infer N> ? (value: N) => R
            : (value: T) => R,
    ): ComputeStream<
        T extends Promise<infer _M> ? R extends Promise<infer _AwaitedR> ? R
        : Promise<R>
    ...
    function mapNN<R>(
        callbackFn: T extends Promise<infer N> ? (value: NonNullable<N>) => R
